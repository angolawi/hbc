...
const getPhaseWorkflows = (phase, repriseTime, estudoTime, aplicacaoTime, revisaoTime, descansoTime) => {
  if (phase === "1") {
    ...
    // Para simplificar as demais fases, usamos os tempos análogos ou mantemos a estrutura delas
    return [
    ...
    // Fase 3 - Somente Aplicação e Enriquecimento
    return [
...
export default function TimerView() {
  ...
  const handlePhaseChange = (e) => {
    ...
    // Auto-adjust default times according to the selected phase recommendation
    if (phase === "1") {
      setRepriseTime(3);
      setEstudoTime(45);
      setAplicacaoTime(10);
      ...
      setRepriseTime(0);
      setEstudoTime(0);
      setAplicacaoTime(60); // 1h de aplicação
      setRevisaoTime(5); // Enriquecimento
      setDescansoTime(0);
    }
  };
  ...
  const proceedToNextStep = () => {
    ...
    if (stepIndex + 1 >= currentSteps.length) {
      alert("Sessão de estudos concluída! Ótimo trabalho!");
      handleCancelSession();
    } else {
  ...
  const handleAdvanceClick = () => {
    if (!canAdvance()) {
      if (currentSteps[stepIndex]?.requiresUpload) {
        alert("Você não pode avançar sem gerar seu material de revisão. Tire uma foto ou faça o upload do seu resumo.");
      } else if (currentSteps[stepIndex]?.requiresCheckbox || currentSteps[stepIndex]?.requiresMultiCheckboxes) {
        alert("Atenção, Você deve preencher os checks obrigatórios de validação de qualidade antes de avançar.");
      }
      return;
    }
    proceedToNextStep();
    ...
    return (
      <section className="animate-in fade-in slide-in-from-bottom-2 duration-500 p-8 flex flex-col min-h-screen bg-zinc-950 items-center justify-center">
        <header className="mb-10 flex flex-col items-center">
          <h1 className="text-5xl font-bold mb-4 tracking-tight py-2 text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Cronometro Mentoria GA</h1>
          <p className="text-zinc-500 text-sm max-w-lg text-center">Configure sua sessão de alto desempenho. Defina a fase do seu estudo e ajuste as durações de acordo com a sua necessidade.</p>
        </header>
        ...
        <Card className="max-w-xl p-10 w-full bg-zinc-900 border-zinc-800 shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              ...
              <Select
                value={repriseTime}
                onChange={e => setRepriseTime(Number(e.target.value))}
                className="bg-zinc-950 border-zinc-800 text-zinc-200"
              >
            ...
            <div>
              <label className="block text-zinc-400 font-medium text-sm mb-2">Estudo/Revisão Inicial (min)</label>
              <Select
                value={estudoTime}
                onChange={e => setEstudoTime(Number(e.target.value))}
                className="bg-zinc-950 border-zinc-800 text-zinc-200"
              >
                {[0, 3, 5, 10, 15, 20, 30, 40, 45, 60].map(val => <option key={val} value={val}>{val} min</option>)}
              </Select>
            ...
            <div>
              <label className="block text-zinc-400 font-medium text-sm mb-2">Aplicação/Teste (min)</label>
              <Select
                value={aplicacaoTime}
                onChange={e => setAplicacaoTime(Number(e.target.value))}
                className="bg-zinc-950 border-zinc-800 text-zinc-200"
              >
                {[0, 3, 5, 10, 15, 20, 30, 40, 45, 60].map(val => <option key={val} value={val}>{val} min</option>)}
              </Select>
            ...
            <div>
              <label className="block text-zinc-400 font-medium text-sm mb-2">Revisão do Resumo (min)</label>
              <Select
                value={revisaoTime}
                onChange={e => setRevisaoTime(Number(e.target.value))}
                className="bg-zinc-950 border-zinc-800 text-zinc-200"
              >
                {[0, 3, 5, 10, 15, 20, 30, 40, 45, 60].map(val => <option key={val} value={val}>{val} min</option>)}
              </Select>
            ...
            <div className="col-span-full">
              ...
              <Select
                value={descansoTime}
                onChange={e => setDescansoTime(Number(e.target.value))}
                className="bg-zinc-950 border-zinc-800 text-zinc-200"
              >
                {[0, 3, 5, 10, 15, 20, 30, 40, 45, 60].map(val => <option key={val} value={val}>{val} min</option>)}
              </Select>
  ...
  return (
    <section className="animate-in fade-in duration-500 min-h-screen flex flex-col p-4 md:p-8 bg-zinc-950 w-full rounded-none">
      <header className="mb-8 flex justify-between items-center bg-zinc-900 p-6 rounded-2xl border border-zinc-800/80 shadow-lg shadow-black/20">
        <div>
      ...
      <div className="flex-grow grid grid-cols-1 xl:grid-cols-3 gap-8 relative w-full">
        <Card className="xl:col-span-2 p-8 md:p-12 flex flex-col items-center justify-center text-center bg-zinc-900 shadow-xl border-zinc-800/50 rounded-3xl overflow-hidden relative">
          ...
          <div className="relative flex items-center justify-center w-[18rem] h-[18rem] md:w-[24rem] md:h-[24rem] my-6">
            <svg viewBox="0 0 100 100" className="absolute w-full h-full -rotate-90 drop-shadow-xl overflow-visible">
              ...
              />
              <circle
                className={isCounting ? "text-indigo-500" : "text-zinc-500"}
                strokeWidth="4"
                ...
                cx="50"
                cy="50"
                style={{ transition: 'stroke-dashoffset 1s linear, color 0.5s ease' }}
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center z-10 space-y-4">
              ...
              {timeRemaining > 0 && currentStepInfo?.duration > 0 && (
                <button onClick={() => setShowTimerText(!showTimerText)} className="text-zinc-500 hover:text-indigo-400 transition-colors p-2.5 rounded-full hover:bg-zinc-800/50 backdrop-blur-md border border-transparent hover:border-zinc-700/50" title={showTimerText ? "Ocultar Timer" : "Mostrar Timer"}>
          ...
          <div className="flex flex-col gap-4 w-full mt-2 items-center">
            {timeRemaining > 0 && (
              <div className="flex gap-4 max-w-md w-full justify-center">
                <Button
                  size="lg"
                  variant={isCounting ? 'outline' : 'primary'}
                  className={`flex-1 ${isCounting ? 'border-zinc-700 text-zinc-300 hover:bg-zinc-800' : 'bg-indigo-600 border-none hover:bg-indigo-500 text-white shadow-xl shadow-indigo-900/20'} ${currentStepInfo?.unskippable && !isCounting && stepIndex === 0 ? 'animate-pulse' : ''} text-lg h-14 rounded-xl`}
                ...
                {!currentStepInfo?.unskippable && (
                  <Button
                    variant="ghost"
                    size="lg"
                    className="text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 h-14 rounded-xl"
                    onClick={() => {
            ...
            {timeRemaining === 0 && (
              <div className="w-full max-w-lg mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {/* Validação Padrão - UPLOAD OU CAMERA */}
                {currentStepInfo?.requiresUpload && (
                  <div className="bg-zinc-950 p-6 rounded-2xl border border-indigo-900/30 mb-6 text-left shadow-xl">
                    <p className="text-sm text-indigo-400 mb-4 font-bold uppercase tracking-widest flex items-center gap-2">
                      MATERIAL BASE
                    </p>
                    <div className="space-y-4">

                      <div className="flex gap-4 w-full">
                        <label className="relative flex-1 bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex flex-col items-center justify-center gap-3 hover:border-indigo-500/50 hover:bg-zinc-800/80 transition-all group cursor-pointer overflow-hidden shadow-inner cursor-pointer">
                          <input
                            type="file"
                            accept=".jpg,.png,.pdf"
                            className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
                            onChange={e => setUploadFileContext(e.target.files[0])}
                          />
                          <div className="bg-indigo-500/10 p-3 rounded-full text-indigo-400 group-hover:scale-110 group-hover:bg-indigo-500 group-hover:text-white transition-all">
                            <Upload size={22} />
                          </div>
                          <span className="text-[11px] font-bold text-zinc-400 group-hover:text-indigo-200 transition-colors text-center uppercase tracking-wider">Upload<br />Arquivo</span>
                        </label>
                        ...
                        <label className="relative flex-1 bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex flex-col items-center justify-center gap-3 hover:border-indigo-500/50 hover:bg-zinc-800/80 transition-all group cursor-pointer overflow-hidden shadow-inner cursor-pointer">
                          ...
                          <span className="text-[11px] font-bold text-zinc-400 group-hover:text-indigo-200 transition-colors text-center uppercase tracking-wider">Tirar<br />Foto</span>
                        </label>
                      </div>

                      {uploadFileContext && (
                        <div className="animate-in fade-in text-xs text-emerald-400 font-bold text-center bg-emerald-950/30 p-3 rounded-xl border border-emerald-900/50 flex flex-col items-center gap-1 shadow-inner">
                          <span className="text-[10px] uppercase tracking-widest opacity-70">Arquivo Anexado Pronto</span>
                          <span className="truncate max-w-[200px] text-emerald-300">{uploadFileContext.name}</span>
                        </div>
                      )}

                      <div className="flex items-center gap-4 opacity-60 px-4">
                        <div className="h-px bg-zinc-800 flex-1"></div>
                        <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">OU DIGITE O RESUMO</span>
                        <div className="h-px bg-zinc-800 flex-1"></div>
                      </div>

                      <textarea
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-sm text-zinc-200 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all placeholder:text-zinc-600 min-h-[120px] resize-none shadow-inner"
                        placeholder="Escreva esquematicamente seu resumo novo por aqui, se não utilizou papel."
                        value={uploadTextContext}
                        onChange={e => setUploadTextContext(e.target.value)}
                      />
                    </div>
                  </div>
                )}

                {/* Validação Fase 2 - CHECKBOX ESPECIFICA */}
                {currentStepInfo?.requiresCheckbox && (
                  <div className="w-full bg-indigo-900/10 p-6 rounded-2xl border border-indigo-500/20 mb-6 flex items-center gap-4 cursor-pointer hover:bg-indigo-900/20 transition-all duration-300 group" onClick={() => setSingleCheckboxCtx(!singleCheckboxCtx)}>
                    <div className={`w-8 h-8 flex-shrink-0 rounded-lg border-2 flex items-center justify-center transition-colors ${singleCheckboxCtx ? 'bg-indigo-500 border-indigo-500' : 'bg-zinc-900 border-zinc-700 group-hover:border-indigo-500/50'}`}>
                      {singleCheckboxCtx && <svg className="w-5 h-5 text-zinc-950" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                    </div>
                    <label className="text-zinc-300 font-medium cursor-pointer flex-1 text-left leading-relaxed text-sm">
                      Confirmo, por minha honra, que já anotei os órgãos/anos responsáveis no verso do meu resumo master.
                    </label>
                  </div>
                )}

                {/* Validação Fase 3 - CHECKBOX MÚLTIPLA */}
                {currentStepInfo?.requiresMultiCheckboxes && (
                  <div className="w-full bg-zinc-950 p-6 rounded-2xl border border-zinc-800/80 mb-6 space-y-3 text-left shadow-lg">
                    <p className="text-sm text-indigo-400 mb-4 font-bold uppercase tracking-widest">SETUP OBRIGATÓRIO (PREENCHA TODOS)</p>

                    {['banca', 'disciplina', 'anos'].map((key) => (
                      <label key={key} className="flex items-center gap-4 cursor-pointer group p-4 rounded-xl border border-zinc-800 hover:border-indigo-500/30 hover:bg-zinc-900/50 transition-all">
                        <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors ${multiCheckboxesCtx[key] ? 'bg-indigo-500 border-indigo-500' : 'bg-zinc-900 border-zinc-700 group-hover:border-indigo-500/50'}`}>
                          {multiCheckboxesCtx[key] && <svg className="w-4 h-4 text-zinc-950" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                        </div>
                ...
                <Button
                  ...
                  className={`w-full h-16 text-lg font-bold tracking-wide shadow-xl overflow-hidden relative group rounded-2xl ${canAdvance() ? 'bg-indigo-600 hover:bg-indigo-500 text-white border-0' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-500 border border-zinc-700'}`}
                >
                  {canAdvance() ? (
                  ...
                  {canAdvance() && <div className="absolute inset-0 z-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:animate-[shimmer_1s_infinite]"></div>}
                </Button>
              </div>
            )}
        ...
        <Card className="p-8 bg-zinc-900/80 border-zinc-800/50 w-full h-fit sticky top-8 shadow-xl rounded-3xl backdrop-blur-md">
          ...
          <div className="space-y-0 relative pl-2">
            <div className="absolute left-[15px] top-6 bottom-10 w-[2px] bg-zinc-800/80 (z-0) rounded-full"></div>
            {currentSteps.map((step, idx) => {
              ...
              if (isCompleted) {
                ...
                lineEffect = "text-decoration-line-through opacity-50";
              } else if (isCurrent) {
                stateClass = "text-zinc-100";
                dotClass = "bg-indigo-500 border-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.5)]";
              }
              ...
              return (
                <div key={idx} className={`relative flex gap-6 pb-12 z-10 ${stateClass} transition-all duration-500 group`}>
                  <div className="mt-1 flex-none relative">
                    ...
                    {isCompleted && (
                      <div className="absolute inset-0 flex items-center justify-center z-20 text-indigo-300">
                    ...
                    <div className={`w-6 h-6 rounded-full border-[3px] ${dotClass} transition-all duration-300 ${isCurrent ? 'ring-4 ring-indigo-500/20' : ''}`}></div>
                  </div>
                  <div>
                    <h4 className={`font-bold tracking-tight transition-all duration-300 ${isCurrent ? 'text-xl text-indigo-100 mb-1' : 'text-base group-hover:text-zinc-400'} ${lineEffect}`}>
                      {step.title}
                    </h4>
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-[10px] font-black text-zinc-500 bg-zinc-950 px-2.5 py-1 rounded-md border border-zinc-800/80 uppercase tracking-widest shadow-inner">
                        {step.duration === 0 ? 'CHECK' : `${step.duration / 60} MIN`}
                      </span>
                    </div>
                  </div>
                </div>
...
function Badge({ label, className }) {
  return (
    <div className={`inline-flex items-center justify-center text-xs font-bold uppercase tracking-wider rounded-full ${className}`}>
      {label}
    </div>
  )
}
